  ...
  constructor(scope: Construct, id: string, props?: AwsOcppGatewayStackProps) {
    ...
    const deleteThing = new lambda.Function(this, 'DeleteThing', {
      ...
      code: lambda.Code.fromAsset(
        path.join(__dirname, '../src/lambdas')
      ),
      environment: {
    ...
    const messageProcessor = new lambda.Function(this, 'OCPPMessageProcessor', {
      ...defaultLambdaProps,
      handler: 'messages_lambda_handler.lambda_handler',
      code: lambda.Code.fromAsset(
        path.join(__dirname, '../src/lambdas'),
        {
          bundling: {
    ...
    const remoteStartTransaction = new lambda.Function(this, 'RemoteStartTransaction', {
      ...defaultLambdaProps,
      handler: 'remote_start_transaction.lambda_handler',
      code: lambda.Code.fromAsset(
        path.join(__dirname, '../src/lambdas')
      ),
    ...
    remoteStartTransaction.role?.attachInlinePolicy(
      new iam.Policy(this, 'PublishToChargePointPolicy', {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            resources: [
    ...
    // Create a Function URL for the remote start transaction Lambda
    const remoteStartTransactionUrl = remoteStartTransaction.addFunctionUrl({
      authType: lambda.FunctionUrlAuthType.NONE, // No auth for simplicity, add IAM auth for production
      cors: {
        allowedOrigins: ['*'],
    ...
    new cdk.CfnOutput(this, 'RemoteStartTransactionUrl', {
      value: remoteStartTransactionUrl.url,
      description: 'URL for the Remote Start Transaction API',
      exportName: 'RemoteStartTransactionUrl',
    });

    // Remote Stop Transaction Lambda
    const remoteStopTransaction = new lambda.Function(this, 'RemoteStopTransaction', {
      ...defaultLambdaProps,
      handler: 'remote_stop_transaction.lambda_handler',
      code: lambda.Code.fromAsset(
        path.join(__dirname, '../src/lambdas')
      ),
      timeout: cdk.Duration.seconds(60),
    ...
    remoteStopTransaction.role?.attachInlinePolicy(
      new iam.Policy(this, 'PublishToChargePointStopPolicy', {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            resources: [
    ...
    // Create a Function URL for the remote stop transaction Lambda
    const remoteStopTransactionUrl = remoteStopTransaction.addFunctionUrl({
      authType: lambda.FunctionUrlAuthType.NONE, // No auth for simplicity, add IAM auth for production
      cors: {
        allowedOrigins: ['*'],
        allowedMethods: [lambda.HttpMethod.POST],
    ...
    // Output the Function URL
    new cdk.CfnOutput(this, 'RemoteStopTransactionUrl', {
      value: remoteStopTransactionUrl.url,
      description: 'URL for the Remote Stop Transaction API',
      exportName: 'RemoteStopTransactionUrl',
    });

...
def remote_start_transaction(charge_point_id, id_tag=None, connector_id=1):
    ...
    Args:
        ...
        id_tag (str, optional): ID tag for authorization. Defaults to None.
    ...
    if id_tag:
        payload["idTag"] = id_tag  # Will be converted to idToken structure in the Lambda
...
def main():
    ...
    parser.add_argument('--transaction-id', '-t', type=str, help='Transaction ID (required for stop)')
    ...
    if args.command == 'start':
        ...
        if result:
            print(f"RequestStartTransaction sent successfully:")
        ...
        if result:
            print(f"RequestStopTransaction sent successfully:")
            print(json.dumps(result, indent=2))
